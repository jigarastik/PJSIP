...
pj_status_t registerAccount(char *pUserName, char *pContactName, char *pPassword, char *pServer)
{
	pj_status_t status;
	pjsua_acc_config acc_cfg;
	const char *uname = pUserName;
	const char *authname;
	const char *contactname = pContactName;
	const char *passwd = pPassword;
	const char *server = pServer;
		...
		acc_cfg.id.slen = pj_ansi_snprintf(acc_cfg.id.ptr, PJSIP_MAX_URL_SIZE,
			...
			if ((status = pjsua_verify_sip_url(acc_cfg.id.ptr)) != 0)
			{
				PJ_LOG(1,(THIS_FILE, "Error: invalid SIP URL '%s' in local id argument",
						  acc_cfg.id));
//				[self displayParameterError: @"Invalid value for username or server."];
				return status;
			}
